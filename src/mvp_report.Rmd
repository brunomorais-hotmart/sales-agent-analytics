...
etl_data$sales_agent_phone_user_number <- str_replace(
  etl_data$sales_agent_phone_user_number,
  "^(.{4})(.+)(.{4})$",
  "\\1****\\3"
)

etl_data$sales_agent_phone_system <- str_replace(
  etl_data$sales_agent_phone_system,
  "^(.{4})(.+)(.{4})$",
  "\\1****\\3"
)
...
```{r}
leads_reached <-nrow(etl_data[etl_data$is_sales_agent_transaction_approved  == 1,])
...
```{r}
conversion_rate <- round((nrow(etl_data[etl_data$is_sales_agent_transaction_approved  == 1,])/nrow(etl_data)) * 100, 2)
...
```{r}
conversion_rate_direct <- round((nrow(etl_data[etl_data$is_sales_agent_transaction_approved  == 1 & etl_data$is_sales_agent_sck == 1, ])/nrow(etl_data)) * 100, 2)
