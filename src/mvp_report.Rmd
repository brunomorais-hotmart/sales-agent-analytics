...
source("src/astrobox-api/hotmart_utils.R")
source("src/astrobox-api/hotmart_auth.R")
source("src/astrobox-api/hotmart_api.R") 
...
table_sales_data <- etl_data %>%
  ...
  arrange(desc(transaction_creation_datetime)) %>%
  mutate(
    transaction_creation_datetime = as.Date(transaction_creation_datetime),
    is_sales_agent_sck = ifelse(is_sales_agent_sck == 1, "Sim", "Não")
  ) %>%
  rename(
     "Data de Transação"= transaction_creation_datetime,
     "HP da Transação"= transaction_reference,
     "Valor"= transaction_item_value,
